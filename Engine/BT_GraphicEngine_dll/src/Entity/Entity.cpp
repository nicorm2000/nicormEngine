...
void Entity::RemoveNode(string nodeName)
{
	for (list<Entity*>::iterator it = nodes.begin(); it != nodes.end(); ++it)
	{
		if ((*it)->name == nodeName)
...
void Entity::RemoveNode(int nodeIndex)
{
	...
	{
		list<Entity*>::iterator it = nodes.begin();
...
void Entity::SetPosition(glm::vec3 position)
{
	...
	if (parent != nullptr)
	{
		transform.position = parent->GetPosition() + transform.localPosition;
	}
	else
	{
...
void Entity::SetRotation(glm::vec3 rotation)
{
	...
	if (parent != nullptr)
	{
		transform.eulerAngles = parent->GetRotation() + transform.localEulerAngles;
	}
	else
	{
	...
	glm::mat4 rotationX = glm::rotate(glm::mat4(1.f), glm::radians(rotation.x), glm::vec3(1.f, 0.f, 0.f));
	glm::mat4 rotationY = glm::rotate(glm::mat4(1.f), glm::radians(rotation.y), glm::vec3(0.f, 1.f, 0.f));
	glm::mat4 rotationZ = glm::rotate(glm::mat4(1.f), glm::radians(rotation.z), glm::vec3(0.f, 0.f, 1.f));
...
{
	SetPosition(glm::vec3(x, GetPositionY(), GetPositionZ()));
}

void Entity::SetPositionY(float y)
{
	SetPosition(glm::vec3(GetPositionX(), y, GetPositionZ()));
}

void Entity::SetPositionZ(float z)
{
	SetPosition(glm::vec3(GetPositionX(), GetPositionY(), z));
}
...
{
	SetRotation(glm::vec3(x, GetRotationY(), GetRotationZ()));
}

void Entity::SetRotationY(float y)
{
	SetRotation(glm::vec3(GetRotationX(), y, GetRotationZ()));
}

void Entity::SetRotationZ(float z)
{
	SetRotation(glm::vec3(GetRotationX(), GetRotationY(), z));
}
...
{
	SetPosition(GetPosition(mat));

	//SetRotation(ToEulerRad(GetRotationByMatrix(mat)));
	//SetRotation(GetRotation(mat));
...
{
	//SetRotation(ToEulerRad(GetRotationByMatrix(glm::lookAt(transform.position, target, transform.up))));
	//SetRotation(GetRotation(glm::lookAt(transform.position, target, transform.up)));
}
...
list<Entity*> Entity::GetNodes()
{
	return nodes;
}

Entity* Entity::GetNode(string nodeName)
{
	if (name == nodeName)
	{
		return this;
...
glm::vec3 Entity::GetPosition(glm::mat4 mat)
{
	return glm::vec3(mat[0].w, mat[1].w, mat[2].w);
}

glm::vec3 Entity::GetRotation(glm::mat4 mat)
{
