...
bool Tilemap::ImportTileMap(std::string filePath, std::string imagePath)
{
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < columns; j++)
		{
			...
			newTile.SetId(_id);
			newTile.SetRender(renderer);
			newTile.LoadTexture(imagePath.c_str(), false, TEXTURE_TYPE::BASE);
...
{
	float convertedPosX = entity->GetPositionX() + (width / 2) * tileWidth;
	float convertedPosY = entity->GetPositionY() - (height / 2) * tileHeight;
	...
	for (int i = left_tile; i <= right_tile; i++)
	{
		for (int j = top_tile; j <= bottom_tile; j++)
		{
			for (int k = 0; k < grid.size(); k++)
			{
				...
				{
					CollisionManager::CollisionUpdate(&grid[k][j][i], entity);
				}
			}
		}
	}
