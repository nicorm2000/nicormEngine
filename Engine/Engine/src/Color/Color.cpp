...
{
	this->R = glm::clamp(static_cast<float>(R), 0.0f, colorValue) / colorValue;
	this->G = glm::clamp(static_cast<float>(G), 0.0f, colorValue) / colorValue;
	this->B = glm::clamp(static_cast<float>(B), 0.0f, colorValue) / colorValue;
}

void Color::SetColorRGBA(int R, int G, int B, int A)
{
	SetColorRGB(R, G, B);
	this->A = glm::clamp(static_cast<float>(A), 0.0f, colorValue) / colorValue;
}
